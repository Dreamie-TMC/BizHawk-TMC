...
namespace MinishCapTools.Elements
{
    ...
    {
        public static List<byte> LoadMemoryRegionAsByteArray(long address, MemApi memory, int length,
            MemoryDomain domain)
        {
            return memory.ReadByteRange(address, length, domain.GetDomainAsString());
        }
        
        public static string ReadString(long address, MemApi memory, int length, MemoryDomain domain)
        {
            return System.Text.Encoding.ASCII.GetString(memory.ReadByteRange(address, length, domain.GetDomainAsString()).ToArray());
        }

        public static uint LoadAddress(long address, MemApi memory, ReadType type, MemoryDomain domain)
        {
        ...
        public static IDictionary<string, uint> LoadCurrentAreaAndRoom(MemApi memory)
        {
        ...
        public static IDictionary<string, int> LoadTextboxValues(MemApi memory)
        {
